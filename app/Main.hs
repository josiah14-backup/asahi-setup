...
writeHyprlandConfig = do
  ...
  let configDir = homeDir </> ".config/hypr"
      ...
      orientationScript = "cycle-master-orientation.sh"
  alreadyExists <- testfile configPath
  ...
  -- Deployed unconditionally, unlike hyprland.conf above: mod-Return in
  -- this repo's hyprland.conf calls this script directly regardless of
  -- whether the config file itself was left untouched, so it needs to
  -- exist either way.
  mktree scriptsDir
  cp (curdir </> "hypr/scripts" </> fromText orientationScript) (scriptsDir </> fromText orientationScript)
  shells ("chmod +x " <> format fp (scriptsDir </> fromText orientationScript)) empty
